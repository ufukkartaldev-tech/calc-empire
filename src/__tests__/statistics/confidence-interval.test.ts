...
describe('Confidence Interval', () => {
  ...
  it('wider CI for lower confidence level (90% narrower than 99%)', () => {
    const ci90 = confidenceInterval({ mean: 100, stdDev: 15, n: 100, confidence: 0.9 });
  ...
  it('throws for n < 1', () => {
    expect(() => confidenceInterval({ mean: 100, stdDev: 15, n: 0, confidence: 0.95 })).toThrow();
  });

  it('throws for confidence outside (0, 1)', () => {
    expect(() => confidenceInterval({ mean: 100, stdDev: 15, n: 30, confidence: 1.5 })).toThrow();
  });
});
