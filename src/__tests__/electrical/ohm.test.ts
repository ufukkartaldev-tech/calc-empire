...
describe("Ohm's Law – Power", () => {
  ...
  it('throws when string or invalid types are provided', () => {
    ...
    expect(() => ohmPower({ voltage: '12V' as any, current: 2 })).toThrow(
  ...
  it('throws when negative resistance is given', () => {
    expect(() => ohmPower({ voltage: 10, resistance: -5 })).toThrow(
      'Resistance (R) cannot be negative.'
    );
  });

  it('throws when negative power is given', () => {
    expect(() => ohmPower({ voltage: 10, power: -50 })).toThrow(
      'Power (P) cannot be negative in passive systems.'
    );
  });
});
