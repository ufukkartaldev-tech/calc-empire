...
describe('Bode Plot Calculator', () => {
  ...
  it('throws if neither C nor L is provided', () => {
    expect(() => calculateBodePlot({ type: 'low-pass', R: 1000 })).toThrow(
      'Either valid C or L must be provided'
    );
  });

  it('throws if R is negative or zero', () => {
    expect(() => calculateBodePlot({ type: 'low-pass', R: 0, C: 1e-6 })).toThrow(
      'Resistance must be positive'
    );
    expect(() => calculateBodePlot({ type: 'low-pass', R: -10, C: 1e-6 })).toThrow(
      'Resistance must be positive'
    );
  });

  it('throws if C or L is negative', () => {
    expect(() => calculateBodePlot({ type: 'low-pass', R: 1000, C: -1e-6 })).toThrow(
      'Capacitance or Inductance must be positive'
    );
    expect(() => calculateBodePlot({ type: 'low-pass', R: 1000, L: -10e-3 })).toThrow(
      'Capacitance or Inductance must be positive'
    );
  });
});
