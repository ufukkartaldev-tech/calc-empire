...
describe('Kirchhoff Circuit Analysis (2-Loop)', () => {
    ...
    it('throws if any resistance is negative', () => {
        expect(() => solveKirchhoff2Loop({ V1: 10, V2: 10, R1: -10, R2: 10, R3: 10 })).toThrow("Resistances cannot be negative");
    });

    it('throws if determinant is zero (e.g. all resistors zero)', () => {
        expect(() => solveKirchhoff2Loop({ V1: 10, V2: 10, R1: 0, R2: 0, R3: 0 })).toThrow("Circuit has no unique valid solution (determinant is zero)");
    });
});
