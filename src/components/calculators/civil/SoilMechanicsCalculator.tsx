        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-8">
            ...
            <div className="p-6 bg-emerald-50/50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-900/30 rounded-3xl">
              <p className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-[0.2em] mb-2">Scientific Basis</p>
              <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed font-medium">
                ...
                <code className="block mt-2 font-mono text-emerald-600 dark:text-emerald-400 bg-white dark:bg-slate-900 p-2 rounded-lg border border-emerald-100 dark:border-emerald-900">τ = c + σ · tan(φ)</code>
              </p>
            </div>
          </div>
          ...
          <div className="space-y-8 flex flex-col">
             ...
             <div className="flex-1 min-h-[300px] bg-slate-50 dark:bg-slate-950 rounded-[48px] border border-slate-100 dark:border-slate-800 relative overflow-hidden flex flex-col items-center justify-center p-8 shadow-inner">
                <svg viewBox="0 0 400 300" className="w-full max-w-[360px]">
                   {/* Axes */}
                   <line x1="40" y1="260" x2="380" y2="260" stroke="currentColor" className="text-slate-300 dark:text-slate-700" strokeWidth="2" />
                   <line x1="40" y1="20" x2="40" y2="260" stroke="currentColor" className="text-slate-300 dark:text-slate-700" strokeWidth="2" />
                   
                   {/* Labels */}
                   <text x="380" y="280" textAnchor="end" className="text-[10px] font-bold fill-slate-400 uppercase">σ (Normal)</text>
                   <text x="30" y="20" textAnchor="end" transform="rotate(-90, 30, 20)" className="text-[10px] font-bold fill-slate-400 uppercase">τ (Shear)</text>
                   ...
                   {results && (
                     <>
                        ...
                        <circle 
                          ...
                          r="5" 
                          fill={200 > (260 - (results.c + ((results.sigma / (results.sigma + 50)) * 300) * Math.tan(results.phi * Math.PI / 180))) ? "#ef4444" : "#3b82f6"} 
                          className="transition-all duration-500"
                        />
                        
                        {/* Cohesion Intercept Label */}
                        <text x="35" y={260 - results.c} textAnchor="end" className="text-[9px] font-bold fill-emerald-600">c</text>
                     </>
                   )}
                </svg>
                
                <div className="absolute bottom-6 text-center">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{t('failureEnv')}</p>
                  <p className="text-[8px] font-mono text-slate-300 dark:text-slate-600">Geometric stability analysis</p>
                </div>
             </div>

             {/* Result Card */}
             <div className="p-8 bg-emerald-600 rounded-[36px] shadow-2xl shadow-emerald-500/20 text-white relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-8 opacity-10 text-9xl pointer-events-none font-black select-none">τ</div>
                  ...
                  <div className="relative z-10 space-y-2">
                    ...
                    <div className="flex items-baseline gap-2">
                      <span className="text-5xl font-mono font-black tracking-tighter">
                        {results.shearStrength.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                      </span>
