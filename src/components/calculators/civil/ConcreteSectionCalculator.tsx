          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg font-mono focus:ring-2 focus:ring-blue-100 outline-none text-slate-900 dark:text-slate-100"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg font-mono focus:ring-2 focus:ring-blue-100 outline-none text-slate-900 dark:text-slate-100 cursor-pointer"
              >
            ...
            <div>
              ...
              <select
                value={fyk}
                onChange={(e) => setFyk(Number(e.target.value))}
                className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg font-mono focus:ring-2 focus:ring-blue-100 outline-none text-slate-900 dark:text-slate-100 cursor-pointer"
              >
            ...
            {results && (
              <svg width={svgW} height={svgH} className="mt-4 drop-shadow-sm">
                ...
                {!results.isOverReinforced && (
                  <>
                    <path
                      d={`M 200 20 L 260 20 L 260 ${20 + safeDrawA} L 200 ${20 + safeDrawA}`}
                      fill="none"
                      stroke="#ef4444"
                      strokeWidth="2"
                    />
                    ...
                    <line
                      ...
                      stroke="#3b82f6"
                      strokeWidth="2"
                      markerStart="url(#force-arrow-left)"
                    />
                    <text
                      x="110"
                      y={25 + drawD}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm">
                ...
                <p className="text-xl font-bold text-slate-900 dark:text-white flex items-baseline gap-1">
                  {results.Md.toFixed(1)}{' '}
                  <span className="text-xs font-normal text-slate-400">kN.m</span>
                </p>
              </div>
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm">
                <p className="text-[11px] text-slate-500 font-semibold uppercase tracking-wider mb-1">
              ...
              <div
                className={`col-span-2 lg:col-span-1 rounded-xl p-4 shadow-sm border ${results.isDuctile ? 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-200 dark:border-emerald-800' : 'bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-800'}`}
              >
                <p
                  className={`text-[11px] font-semibold uppercase tracking-wider mb-1 ${results.isDuctile ? 'text-emerald-700 dark:text-emerald-500' : 'text-red-700 dark:text-red-500'}`}
                >
                  {t('ductility')}
                </p>
                <div className="flex items-center gap-2">
                  <span className="text-xl">{results.isDuctile ? '✅' : '⚠️'}</span>
                  <p
                    className={`text-sm font-bold ${results.isDuctile ? 'text-emerald-800 dark:text-emerald-400' : 'text-red-800 dark:text-red-400'}`}
                  >
