...
export function DataVisualizer() {
  ...
  const [input, setInput] = useState('12, 15, 14, 16, 18, 20, 22, 21, 25, 24, 28, 30, 32, 35, 38, 40, 42, 45, 48, 50');
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('bins')}</label>
            ...
            <div className="p-4 bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-900/30 rounded-2xl space-y-3">
              <h4 className="text-[10px] font-black text-emerald-600 uppercase tracking-wider">{t('summary')}</h4>
              <div className="grid grid-cols-2 gap-2">
                <div className="text-center p-2 bg-white dark:bg-slate-900 rounded-lg border border-emerald-100 dark:border-emerald-800/50">
                   ...
                   <p className="text-sm font-black text-slate-700 dark:text-slate-200">{numbers.length}</p>
                </div>
                <div className="text-center p-2 bg-white dark:bg-slate-900 rounded-lg border border-emerald-100 dark:border-emerald-800/50">
          ...
          <div className="bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-100 dark:border-slate-800 p-6 flex flex-col h-full">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest">{t('histogram')}</h3>
            </div>
              ...
              <div className="flex-1 min-h-[400px] flex items-end gap-1 px-4 relative">
                ...
                <div className="absolute inset-0 flex flex-col justify-between pointer-events-none pb-12 pt-4 pr-4">
                  {[0, 0.25, 0.5, 0.75, 1].map(lvl => (
                    <div key={lvl} className="w-full border-t border-slate-200 dark:border-slate-800 flex items-center">
                ...
                <div className="flex-1 flex items-end gap-1 h-full pb-10">
                  {histogramData.map((d, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center group relative h-full justify-end">
                      ...
                      <span className={`absolute bottom-2 text-[9px] font-mono font-bold text-slate-400 ${histogramData.length > 12 ? '-rotate-45 -translate-x-1 translate-y-2' : ''}`}>
                        {d.binLabel}
                      </span>
                    </div>
                  ))}
