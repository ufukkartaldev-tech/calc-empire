...
export function BodePlotVisualizer({ className = '' }: BodePlotVisualizerProps) {
  ...
  const drawGrid = (minY: number, maxY: number, ticks: number, isPhase: boolean = false) => {
    ...
    if (plotData) {
      ...
      for (let log = logMin; log <= logMax; log++) {
        const x = padX + ((log - Math.log10(fMin)) / (Math.log10(fMax) - Math.log10(fMin))) * (svgW - 2 * padX);
        if (x >= padX && x <= svgW - padX) {
          lines.push(<line key={`x-${log}`} x1={x} y1={padY} x2={x} y2={svgH - padY} stroke="#94a3b8" strokeDasharray="2,2" strokeWidth="0.5" className="dark:stroke-slate-700" />);
          if (log === logMin || log === logMax || log === Math.round(logMin + (logMax - logMin) / 2)) {
            lines.push(<text key={`tx-${log}`} x={x} y={svgH - 5} fontSize="9" textAnchor="middle" className="fill-slate-500 font-mono">10^{log}</text>);
          }
        }
      }
    }
    ...
    for (let i = 0; i <= ticks; i++) {
      ...
      lines.push(<line key={`y-${i}`} x1={padX} y1={y} x2={svgW - padX} y2={y} stroke="#94a3b8" strokeDasharray="2,2" strokeWidth="0.5" className="dark:stroke-slate-700" />);
      lines.push(<text key={`ty-${i}`} x={padX - 5} y={y + 3} fontSize="9" textAnchor="end" className="fill-slate-500 font-mono">{val.toFixed(0)}{isPhase ? '°' : ''}</text>);
    }
  ...
  return (
    <div className={`w-full max-w-4xl mx-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm ${className}`}>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Bode Plot Visualizer</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800">
        ...
        <div className="ce-field__control">
          <label className="text-xs font-semibold text-slate-500 mb-1 block">R (Resistance Ω)</label>
          <input type="number" value={R} onChange={e => setR(Number(e.target.value))} className="w-full px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg font-mono focus:ring-2 focus:ring-blue-100 outline-none text-slate-900 dark:text-slate-100" />
        </div>

        {systemType === 'rc' ? (
          <div className="ce-field__control">
            <label className="text-xs font-semibold text-slate-500 mb-1 block">C (Capacitance F)</label>
            <input type="number" value={C} step={0.000001} onChange={e => setC(Number(e.target.value))} className="w-full px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg font-mono focus:ring-2 focus:ring-blue-100 outline-none text-slate-900 dark:text-slate-100" />
          </div>
        ) : (
          <div className="ce-field__control">
            <label className="text-xs font-semibold text-slate-500 mb-1 block">L (Inductance H)</label>
            <input type="number" value={L} step={0.001} onChange={e => setL(Number(e.target.value))} className="w-full px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg font-mono focus:ring-2 focus:ring-blue-100 outline-none text-slate-900 dark:text-slate-100" />
          </div>
        )}
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
            <div className="bg-slate-100 dark:bg-slate-800 px-4 py-2 border-b border-slate-200 dark:border-slate-700">
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Magnitude |H(jω)| (dB) vs Frequency</h3>
            </div>
            <div className="p-4 bg-white dark:bg-slate-950 flex justify-center">
              <svg width="100%" height="auto" viewBox={`0 0 ${svgW} ${svgH}`} preserveAspectRatio="xMidYMid meet" className="max-w-2xl">
                ...
                <line
                  ...
                  strokeDasharray="4,4"
                />
                <text x={padX + 0.5 * (svgW - 2 * padX) + 5} y={padY + 15} fontSize="9" fill="#ef4444" fontWeight="bold">-3dB</text>
              </svg>
            </div>
          </div>
          ...
          <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
            <div className="bg-slate-100 dark:bg-slate-800 px-4 py-2 border-b border-slate-200 dark:border-slate-700">
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Phase ∠H(jω) (°) vs Frequency</h3>
            </div>
            <div className="p-4 bg-white dark:bg-slate-950 flex justify-center">
              <svg width="100%" height="auto" viewBox={`0 0 ${svgW} ${svgH}`} preserveAspectRatio="xMidYMid meet" className="max-w-2xl">
                <rect width={svgW} height={svgH} fill="none" />
                {drawGrid(filterType === 'low-pass' ? -90 : 0, filterType === 'low-pass' ? 0 : 90, 4, true)}
                <path
                  d={generatePath(plotData.phases, plotData.frequencies, filterType === 'low-pass' ? -90 : 0, filterType === 'low-pass' ? 0 : 90)}
                  fill="none"
                  stroke="#ec4899"
                  strokeWidth="2"
                />
                <line
                  ...
                  strokeDasharray="4,4"
                />
                <text x={padX + 0.5 * (svgW - 2 * padX) + 5} y={padY + 15} fontSize="9" fill="#ef4444" fontWeight="bold">±45°</text>
              </svg>
            </div>
          </div>
        </div>
