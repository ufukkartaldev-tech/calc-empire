...
export function PowerCalculator() {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
      ...
      <p className="text-slate-500 dark:text-slate-400 mb-8 text-sm">{t('description')}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-6 flex flex-col items-center justify-center relative min-h-[300px]">
            <h3 className="absolute top-4 left-6 text-xs font-semibold text-slate-500 uppercase tracking-widest">
              {t('visualizer')}
            </h3>
            ...
            {powerTriangle && results && (
              ...
              >
                ...
                />
                <text
                  x={powerTriangle.x0 + 30}
                  y={results.reactivePower > 0 ? powerTriangle.y0 - 5 : powerTriangle.y0 + 15}
                  fontSize="10"
                ...
                {/* Labels */}
                <text
                  x={(powerTriangle.x0 + powerTriangle.xP) / 2}
                  y={powerTriangle.y0 + 15}
                  textAnchor="middle"
                  fontSize="10"
                ...
                <text
                  x={powerTriangle.xP + 10}
                  y={(powerTriangle.yP + powerTriangle.yQ) / 2}
                  textAnchor="start"
                  fontSize="10"
                ...
                <text
                  ...
                  textAnchor="end"
                  fontSize="10"
                  className="fill-yellow-600 dark:fill-yellow-400 font-bold"
                >
                  S
                </text>
              </svg>
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-800/40 rounded-lg border border-slate-100 dark:border-slate-800">
              ...
              <span className="text-[10px] font-bold text-slate-500 uppercase">
                {t('activePower').split(' ')[0]}
              </span>
            </div>
            <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-800/40 rounded-lg border border-slate-100 dark:border-slate-800">
              <div className="w-2 h-2 rounded-full bg-blue-500"></div>
              <span className="text-[10px] font-bold text-slate-500 uppercase">
                {t('reactivePower').split(' ')[0]}
              </span>
            </div>
            <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-800/40 rounded-lg border border-slate-100 dark:border-slate-800">
              <div className="w-2 h-2 rounded-full bg-yellow-500"></div>
              <span className="text-[10px] font-bold text-slate-500 uppercase">
                {t('apparentPower').split(' ')[0]}
              </span>
            </div>
          </div>
        </div>
