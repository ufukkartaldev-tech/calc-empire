...
export function ResistorVisualizer({ className = '' }: ResistorVisualizerProps) {
    ...
    return (
        <div className={`w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm ${className}`}>
            ...
            <div className="mb-6">
                <label className="text-sm font-medium text-slate-600 dark:text-slate-400 mb-2 block">{t('bandCount')}</label>
                <div className="flex space-x-2">
                    {[4, 5, 6].map((count) => (
                        <button
                            key={count}
                            onClick={() => handleBandCountChange(count as 4 | 5 | 6)}
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors border ${bandCount === count
            ...
            <div className="mb-6 p-6 bg-slate-50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800 rounded-lg flex justify-center items-center">
                ...
                >
                    ...
                    <rect x="12" y="42" width="76" height="36" fill="#fde68a" stroke="#d97706" strokeWidth="1" rx="6" />
                    ...
                    <path d="M 12 48 C 12 42, 16 42, 16 42 L 16 78 C 16 78, 12 78, 12 72 Z" fill="rgba(0,0,0,0.05)" />
                    <path d="M 88 48 C 88 42, 84 42, 84 42 L 84 78 C 84 78, 88 78, 88 72 Z" fill="rgba(0,0,0,0.05)" />
                </svg>
            </div>
            ...
            {result ? (
                <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800/50 rounded-lg p-5">
                    <h3 className="text-sm font-medium text-emerald-800 dark:text-emerald-400 mb-2 p-0 m-0 leading-none">{t('result')}</h3>
