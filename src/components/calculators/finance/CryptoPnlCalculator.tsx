        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-8">
            ...
            <div className="p-6 bg-amber-50/50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/30 rounded-3xl">
              <p className="text-[10px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-[0.2em] mb-2">Profit Logic</p>
              <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed font-medium">
                P&L is calculated as the difference between exit and entry price multiplied by quantity. Leverage amplifies the percentage return relative to initial margin.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-8 flex flex-col">
             ...
             <div className="flex-1 min-h-[250px] bg-slate-950 rounded-[48px] border-4 border-slate-900 relative overflow-hidden p-8 shadow-2xl flex flex-col justify-end">
                ...
                <div className="relative z-10 space-y-6">
                  <div className="flex justify-between items-center text-white">
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">{t('breakEven')}</span>
                  ...
                  <div className="h-px bg-slate-800 w-full relative">
                    {results && (
                      <div 
                        ...
                        style={{ left: `${Math.min(100, Math.max(0, 50 + (results.pnlPercent / 2)))}%` }}
                      ></div>
                    )}
                  </div>

                  <div className="flex justify-between items-center text-white">
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Live Target</span>
                    <span className="font-mono text-sm">{results?.exit.toLocaleString()} USD</span>
                  </div>
                </div>

                <div className="absolute top-6 right-8">
                   <div className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-tighter ${results && results.totalPnl >= 0 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className={`p-8 rounded-[36px] shadow-2xl relative overflow-hidden transition-colors duration-500 ${results && results.totalPnl >= 0 ? 'bg-emerald-600' : 'bg-red-600'} text-white`}>
                   <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-80 mb-1">{t('totalPnl')}</p>
                   <div className="flex items-baseline gap-2">
                     <span className="text-4xl font-mono font-black tracking-tighter">
                       {results ? (results.totalPnl >= 0 ? '+' : '') + results.totalPnl.toLocaleString(undefined, { maximumFractionDigits: 2 }) : '---'}
                     </span>
                ...
                <div className="p-8 bg-slate-900 rounded-[36px] shadow-2xl text-white relative overflow-hidden">
                   <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40 mb-1">{t('roi')}</p>
                   <div className="flex items-baseline gap-2">
                     <span className={`text-4xl font-mono font-black tracking-tighter ${results && results.pnlPercent >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                       {results ? (results.pnlPercent >= 0 ? '+' : '') + results.pnlPercent.toFixed(2) : '---'}%
                     </span>
                   </div>
                   <div className="mt-4 flex flex-col gap-1">
                      <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest leading-none">Init. Margin</span>
                      <span className="text-xs font-mono font-bold text-slate-300">{results ? results.initialMargin.toFixed(2) : '---'} USD</span>
                   </div>
                </div>
             </div>
          </div>
