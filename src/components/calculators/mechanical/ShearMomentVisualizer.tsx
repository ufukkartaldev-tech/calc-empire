        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-8">
            ...
            <div className="p-6 bg-emerald-50/50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-900/30 rounded-3xl space-y-4">
              <h3 className="text-xs font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">{t('reactions')}</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-emerald-100 dark:border-emerald-900/50">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{t('leftReaction')}</p>
                  <p className="text-xl font-mono font-black text-slate-800 dark:text-white">{results ? results.R1.toFixed(2) : '---'} <span className="text-[10px] opacity-40">N</span></p>
                </div>
                <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-emerald-100 dark:border-emerald-900/50">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{t('rightReaction')}</p>
                  <p className="text-xl font-mono font-black text-slate-800 dark:text-white">{results ? results.R2.toFixed(2) : '---'} <span className="text-[10px] opacity-40">N</span></p>
                </div>
              </div>
            </div>
          </div>
              ...
              <>
                ...
                <div className="space-y-2">
                   <div className="flex justify-between items-end px-2">
                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('shearDiagram')}</p>
                     <p className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400">{t('maxShear')}: {Math.max(results.R1, results.R2).toFixed(2)} N</p>
                   </div>
                   <div className="aspect-[4/1] bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-100 dark:border-slate-800 relative overflow-hidden p-4">
                     <svg viewBox="0 0 400 100" className="w-full h-full preserve-3d">
                        <line x1="0" y1="50" x2="400" y2="50" stroke="currentColor" className="text-slate-300 dark:text-slate-700" strokeWidth="1" />
                        {/* SFD Shape */}
                        <path 
                ...
                <div className="space-y-2">
                   <div className="flex justify-between items-end px-2">
                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('momentDiagram')}</p>
                     <p className="text-[10px] font-bold text-blue-600 dark:text-blue-400">{t('maxMoment')}: {results.maxMoment.toFixed(2)} Nm</p>
                   </div>
                   <div className="aspect-[4/1] bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-100 dark:border-slate-800 relative overflow-hidden p-4">
                     <svg viewBox="0 0 400 100" className="w-full h-full">
                        <line x1="0" y1="20" x2="400" y2="20" stroke="currentColor" className="text-slate-300 dark:text-slate-700" strokeWidth="1" />
                        {/* BMD Shape */}
                        <path 
                ...
                <div className="pt-4 mt-auto">
                   <div className="bg-slate-900 rounded-3xl p-6 relative overflow-hidden shadow-2xl">
                     <div className="absolute top-0 right-0 p-4 opacity-5 text-4xl font-black">BEAM</div>
                           ...
                           >
                              <div className="flex flex-col items-center">
                                 ...
                                 <svg width="20" height="30" viewBox="0 0 20 30">
                                    <path d="M 10 0 L 10 25 M 5 20 L 10 25 L 15 20" stroke="#f87171" strokeWidth="2" fill="none" />
                                 </svg>
                              </div>
                           </div>
                           {/* Supports */}
