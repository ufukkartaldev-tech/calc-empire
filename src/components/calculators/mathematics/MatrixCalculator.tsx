...
const MatrixView = ({ m, title, onUpdate, onResize }: MatrixViewProps) => (
  <div className="flex flex-col items-center gap-2">
    ...
    <div className="relative inline-flex p-1">
      ...
      <div className="grid gap-1 px-1" style={{ gridTemplateColumns: `repeat(${m[0].length}, minmax(40px, 1fr))` }}>
    ...
    {onResize && (
      <div className="flex gap-2 mt-1">
        <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-[10px] font-bold text-slate-500">
          Rows: 
          <button onClick={() => onResize(m.length - 1, m[0].length)} className="hover:text-blue-500">−</button>
          <span>{m.length}</span>
          <button onClick={() => onResize(m.length + 1, m[0].length)} className="hover:text-blue-500">+</button>
        </div>
        <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-[10px] font-bold text-slate-500">
          Cols: 
          <button onClick={() => onResize(m.length, m[0].length - 1)} className="hover:text-blue-500">−</button>
...
export function MatrixCalculator() {
  ...
  const result = useMemo(() => {
    ...
    try {
      ...
      if (operation === 'determinant') return calculateDeterminant(matrixA);
    } catch {
      return null;
    }
    return null;
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden p-6">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">Advanced linear algebra solver</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] items-center gap-8">
          {/* Matrix A */}
          <MatrixView m={matrixA} title="Matrix A" onUpdate={(r, c, v) => updateCell('A', r, c, v)} onResize={handleResizeA} />
          ...
          {(operation === 'add' || operation === 'subtract' || operation === 'multiply') ? (
            <MatrixView m={matrixB} title="Matrix B" onUpdate={(r, c, v) => updateCell('B', r, c, v)} onResize={handleResizeB} />
          ) : (
