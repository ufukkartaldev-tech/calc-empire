...
import { useCalculusStore } from '@/stores/calculusStore';
import { normalizeToSvg } from '@/utils/svg-helpers';
...
export function CalculusCalculator() {
  ...
  // Axes calculations (derived from viewBox) - memoized to prevent recalculation
  const axisX0 = useMemo(
    () => normalizeToSvg(0, viewBox.yMin, viewBox.yMax, 250, 0),
    [viewBox.yMin, viewBox.yMax]
  );
  const axisY0 = useMemo(
    () => normalizeToSvg(0, viewBox.xMin, viewBox.xMax, 0, 500),
    [viewBox.xMin, viewBox.xMax]
  );
          ...
          <div className="space-y-6">
            <div className="relative aspect-[2/1] bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-inner">
              <svg viewBox="0 0 500 250" className="w-full h-full">
                ...
                {tangentPoints.length === 2 && (
                  <line
                    x1={tangentPoints[0].x}
                    y1={tangentPoints[0].y}
                    x2={tangentPoints[1].x}
                    y2={tangentPoints[1].y}
                    stroke="#f59e0b"
                    strokeWidth="2"
                    strokeDasharray="4 2"
                  />
                ...
                {!isNaN(fxAtPoint) && (
                  <circle
                    cx={normalizeToSvg(pointX, viewBox.xMin, viewBox.xMax, 0, 500)}
                    cy={normalizeToSvg(fxAtPoint, viewBox.yMin, viewBox.yMax, 250, 0)}
                    r="4"
                    fill="#f59e0b"
                  />
                )}
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-mono italic">
                  f(x) =
                </span>
                <input
                  type="text"
              ...
              <p className="text-[10px] text-slate-400">
                Supports x, sin, cos, tan, exp, log, sqrt, ^, pi, e
              </p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/30 rounded-2xl p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-amber-800 dark:text-amber-400 text-sm italic">
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                <div className="pt-2 border-t border-amber-100 dark:border-amber-900/20">
                  <p className="text-[10px] text-amber-700/60 uppercase font-bold mb-1">
                    Result f&apos;(x)
                  </p>
                  <p className="text-2xl font-mono font-bold text-amber-900 dark:text-amber-200">
                    {isNaN(derivativeResult)
                      ? '—'
                      : derivativeResult.toFixed(6).replace(/\.?0+$/, '')}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-900/30 rounded-2xl p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-indigo-800 dark:text-indigo-400 text-sm italic">
                  ∫ Integration
                </h3>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-indigo-700/60 uppercase">
                    Lower (a)
                  </label>
                  <input
                    type="number"
              ...
              <div className="pt-2 border-t border-indigo-100 dark:border-indigo-900/20">
                <p className="text-[10px] text-indigo-700/60 uppercase font-bold mb-1">
                  Definite Integral
                </p>
